...
if (isset($_POST['delete_all'])) {
   // Assuming $conn is your database connection and $user_id is set appropriately
   ...
   if ($delete_cart_items->execute()) {
      ...
      // Trigger will automatically delete associated rows due to the defined trigger
   } else {
      $message[] = 'Failed to delete items from cart: ' . $conn->error;
   }
   ...
   if (isset($_POST['update_qty'])) {
      ...
      if ($update_qty->execute()) {
         ...
         // Trigger 'after_update_cart_qty' will automatically execute after update
      } else {
         $message[] = 'Failed to update cart quantity: ' . $conn->error;
      }
      $update_qty->close();
}
